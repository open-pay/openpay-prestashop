...
class OpenpayPse extends PaymentModule
{

    private $error = array();
    private $validation = array();
    private $limited_currencies = array('COP');
    ...
    public function __construct()
    {
        ...
        $this->sandbox_url = 'https://sandbox-api.openpay.co/v1';
        $this->url = 'https://api.openpay.co/v1';        
    ...
    public function install()
    {
        ...
        $ret = parent::install() && $this->createPendingState() &&                
                ...
                Configuration::updateValue('OPENPAY_PSE_MODE', 0) &&
                Configuration::updateValue('OPENPAY_PSE_COUNTRY', 'CO') &&                   
                Configuration::updateValue('OPENPAY_PSE_WEBHOOK_ID_TEST', null) &&
                Configuration::updateValue('OPENPAY_PSE_WEBHOOK_ID_LIVE', null) &&
                Configuration::updateValue('OPENPAY_PSE_WEBHOOK_USER', Tools::substr(md5(uniqid(rand(), true)), 0, 10)) &&
                Configuration::updateValue('OPENPAY_PSE_WEBHOOK_PASSWORD', Tools::substr(md5(uniqid(rand(), true)), 0, 10)) &&                
                Configuration::updateValue('OPENPAY_PSE_WEBHOOK_URL', _PS_BASE_URL_.__PS_BASE_URI__) &&               
                $this->installDb();
    ...
    private function createPendingState()
    {
        ...
        $state->module_name = 'openpaypse';
        $templ = array();

        foreach ($languages as $lang) {
            $templ[$lang['id_lang']] = 'openpaypse';
        }
    ...
    {
        /* Eliminar webhooks */
        if (Configuration::get('OPENPAY_PSE_WEBHOOK_ID_TEST') != null) {
            $this->deleteWebhook(Configuration::get('OPENPAY_PSE_WEBHOOK_ID_TEST'), false);
        }

        if (Configuration::get('OPENPAY_PSE_WEBHOOK_ID_LIVE') != null) {
    ...
    public function processPayment()
    {
        ...
        try {
            ...
            if ($result_json->payment_method && $result_json->payment_method->type == 'redirect') {
                ...
                Tools::redirect($result_json->payment_method->url);
            }
            ...
            $this->context->cookie->__set('openpay_pse_error', $e->getMessage());
    ...
    public function offlinePayment($payment_method)
    {
        ...
        $redirect_url = _PS_BASE_URL_.__PS_BASE_URI__.'module/openpaypse/confirm';
        $charge_request = array(
            'method' => $payment_method,
            'currency' => $this->context->currency->iso_code,
            'amount' => $amount,
            'description' => $this->l('PrestaShop Cart ID:').' '.(int) $cart->id,
            'iva' => Configuration::get('OPENPAY_PSE_IVA'),
    ...
    {
        if (Configuration::get('OPENPAY_PSE_MODE')) {
            return Configuration::get('OPENPAY_PSE_PRIVATE_KEY_LIVE') != '';
        } else {
            return Configuration::get('OPENPAY_PSE_PRIVATE_KEY_TEST') != '';
        }
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('SubmitOpenpay')) {
            ...
            $configuration_values = array(
                'OPENPAY_PSE_MODE' => Tools::getValue('openpay_pse_mode'),
                'OPENPAY_PSE_MERCHANT_ID_TEST' => trim(Tools::getValue('openpay_pse_merchant_id_test')),
                'OPENPAY_PSE_MERCHANT_ID_LIVE' => trim(Tools::getValue('openpay_pse_merchant_id_live')),
                'OPENPAY_PSE_PRIVATE_KEY_TEST' => trim(Tools::getValue('openpay_pse_private_key_test')),
                'OPENPAY_PSE_PRIVATE_KEY_LIVE' => trim(Tools::getValue('openpay_pse_private_key_live')),                              
                'OPENPAY_PSE_IVA' => trim(Tools::getValue('openpay_pse_iva')),                              
                'OPENPAY_PSE_WEBHOOK_URL' => trim(Tools::getValue('openpay_pse_webhook_url'))
            );           
            ...
            $mode = Configuration::get('OPENPAY_PSE_MODE') ? 'LIVE' : 'TEST';
            ...
            if (!$this->getMerchantInfo()) {
                ...
                Configuration::deleteByName('OPENPAY_PSE_MERCHANT_ID_'.$mode);
                Configuration::deleteByName('OPENPAY_PSE_PRIVATE_KEY_'.$mode);
                Configuration::deleteByName('OPENPAY_PSE_WEBHOOK_ID_'.$mode);
            }
        }
        ...
        $this->context->smarty->assign(array(
            ...
            'openpay_pse_form_link' => $_SERVER['REQUEST_URI'],
            'openpay_pse_configuration' => Configuration::getMultiple(
                array(
                    'OPENPAY_PSE_MODE',
                    'OPENPAY_PSE_MERCHANT_ID_TEST',
                    'OPENPAY_PSE_MERCHANT_ID_LIVE',
                    'OPENPAY_PSE_PRIVATE_KEY_TEST',
                    'OPENPAY_PSE_PRIVATE_KEY_LIVE',  
                    'OPENPAY_PSE_IVA',                                   
                    'OPENPAY_PSE_WEBHOOK_URL'                    
                )
            ),
            'openpay_pse_ssl' => Configuration::get('PS_SSL_ENABLED'),
            'openpay_pse_validation' => $this->validation,
            'openpay_pse_error' => (empty($this->error) ? false : $this->error),
            'openpay_pse_validation_title' => $validation_title            
        ));
    ...
    public function getOpenpayCustomer($customer_id)
    {
        ...
        $mode = Configuration::get('OPENPAY_PSE_MODE') == 'true' ? 'live' : 'test';
    ...
    private function formatAddress($customer_data, $address) {
        $country = Configuration::get('OPENPAY_PSE_COUNTRY');
        if ($country === 'MX') {
    ...
    {
        $pk = Configuration::get('OPENPAY_PSE_MODE') ? Configuration::get('OPENPAY_PSE_PRIVATE_KEY_LIVE') : Configuration::get('OPENPAY_PSE_PRIVATE_KEY_TEST');
        $id = Configuration::get('OPENPAY_PSE_MODE') ? Configuration::get('OPENPAY_PSE_MERCHANT_ID_LIVE') : Configuration::get('OPENPAY_PSE_MERCHANT_ID_TEST');

        $openpay = Openpay::getInstance($id, $pk, 'CO');
        Openpay::setProductionMode(Configuration::get('OPENPAY_PSE_MODE'));
    ...
    public function createWebhook($force_host_ssl = false)
    {
        ...
        $domain = rtrim(Configuration::get('OPENPAY_PSE_WEBHOOK_URL'), "/");        
        $webhook_data = array(            
            'url' => $domain.'/modules/openpaypse/notification.php',            
        ...
        try {
            ...
            Configuration::updateValue('OPENPAY_PSE_WEBHOOK_ID_'.$mode, $webhook->id);
            return $webhook;
        } catch (Exception $e) {
    ...
    {
        $pk = $mode ? Configuration::get('OPENPAY_PSE_PRIVATE_KEY_LIVE') : Configuration::get('OPENPAY_PSE_PRIVATE_KEY_TEST');
        $id = $mode ? Configuration::get('OPENPAY_PSE_MERCHANT_ID_LIVE') : Configuration::get('OPENPAY_PSE_MERCHANT_ID_TEST');
    ...
    {
        $sk = Configuration::get('OPENPAY_PSE_MODE') ? Configuration::get('OPENPAY_PSE_PRIVATE_KEY_LIVE') : Configuration::get('OPENPAY_PSE_PRIVATE_KEY_TEST');
        $id = Configuration::get('OPENPAY_PSE_MODE') ? Configuration::get('OPENPAY_PSE_MERCHANT_ID_LIVE') : Configuration::get('OPENPAY_PSE_MERCHANT_ID_TEST');

        $url = (Configuration::get('OPENPAY_PSE_MODE') ? $this->url : $this->sandbox_url).'/'.$id;
        ...
        if ($dhvalue = opendir($directory)) {
            while (($file = readdir($dhvalue)) !== false) {
                if (is_dir($directory.$file) && $file[0] != '.') {
                    ...
                    $html_file_origin = _PS_MODULE_DIR_.$this->name.'/mails/'.$file.'/openpaypse.html';
                    $txt_file_origin = _PS_MODULE_DIR_.$this->name.'/mails/'.$file.'/openpaypse.txt';
                    ...
                    $html_file_destination = $directory.$file.'/openpaypse.html';
                    $txt_file_destination = $directory.$file.'/openpaypse.txt';
