...
class OpenpayStores extends PaymentModule {
    ...
    public function __construct() {
        ...
        $this->author = 'Openpay';
        parent::__construct();
        $this->displayName = $this->l('Openpay Tiendas');
        $this->description = $this->l('Accept cash payments with Openpay (7 Eleven, Extra, Farmacias del Ahorro, Farmacias Benavides, etc.)');
    ...
    public function install() {
        ...
        $ret = parent::install() && $this->_createPendingState() && 
                ...
                Configuration::updateValue('OPENPAY_STORE_MODE', 0) && 
                $this->installDb();        
    ...
    public function installDb() {
		...
		CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'openpay_customer` (
               ...
               Db::getInstance()->Execute('
                CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'openpay_cash_transaction` (
    ...
    public function uninstall() {
        return parent::uninstall() && 
                Configuration::deleteByName('OPENPAY_STORE_PUBLIC_KEY_TEST') &&
                Configuration::deleteByName('OPENPAY_STORE_PUBLIC_KEY_LIVE') &&
                Configuration::deleteByName('OPENPAY_STORE_MERCHANT_ID_LIVE') &&
                Configuration::deleteByName('OPENPAY_STORE_MERCHANT_ID_TEST') &&
                Configuration::deleteByName('OPENPAY_STORE_MODE') &&
                Configuration::deleteByName('OPENPAY_STORE_PRIVATE_KEY_TEST') && 
                Configuration::deleteByName('OPENPAY_STORE_PRIVATE_KEY_LIVE') &&
                Configuration::deleteByName('OPENPAY_STORE_DEADLINE_TEST') &&
                Configuration::deleteByName('OPENPAY_STORE_DEADLINE_LIVE') &&
                Db::getInstance()->Execute('DROP TABLE `' . _DB_PREFIX_ . 'openpay_cash_transaction`');
    }
    ...
    public function hookPayment($params) {
        ...
        $this->smarty->assign('validation_url', (Configuration::get('PS_SSL_ENABLED') ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . __PS_BASE_URI__ . 'index.php?process=validation&fc=module&module=openpaystores&controller=default');
        
        
        $this->smarty->assign('stores', $this->_path . 'img/stores_logo.png');
    ...
    public function hookAdminOrder($params) {
        ...
        if (Db::getInstance()->getValue('SELECT module FROM ' . _DB_PREFIX_ . 'orders WHERE id_order = ' . (int) $id_order) == $this->name) {
            $openpay_transaction_details = Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'openpay_cash_transaction WHERE id_order = ' . (int) $id_order . ' AND type = \'payment\'');
            ...
            if (isset($openpay_transaction_details['id_transaction'])) {
                $output .= '
                        ...
                        '<b>'.$this->l('Barcode: ').'</b><img src="'.Tools::safeOutput($openpay_transaction_details['barcode']).'"/><br>'.
                        '<b>'.$this->l('Reference: ').'</b>'.Tools::safeOutput($openpay_transaction_details['reference']).' <br> </p>';
            } else
    ...
    public function hookBackOfficeHeader() {
        ...
        if (Db::getInstance()->getValue('SELECT module FROM ' . _DB_PREFIX_ . 'orders WHERE id_order = ' . (int) $id_order) == $this->name) {
            $openpay_transaction_details = Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'openpay_cash_transaction WHERE id_order = ' . (int) $id_order . ' AND type = \'payment\' AND status = \'paid\'');
            $output = '
    ...
    public function hookPaymentReturn($params) {
        ...
        $this->smarty->assign(
                'openpay_order', 
                array(
                    'barcode' => $openpay_transaction_details['reference'], 
                    'barcode_url' => $openpay_transaction_details['barcode'], 
                    'amount'=>$openpay_transaction_details['amount'], 
                    'currency'=>$openpay_transaction_details['currency'],
                    'email' => $customer->email,
                    'date' => $this->getLongGlobalDateFormat($openpay_transaction_details['date_add']),
    ...
    public function processPayment() {
        ...
        try {
            ...
            $deadline = (Configuration::get('OPENPAY_STORE_MODE')) ? Configuration::get('OPENPAY_STORE_DEADLINE_LIVE') : Configuration::get('OPENPAY_STORE_DEADLINE_TEST');
            ...
            $message = $this->l('Openpay Transaction Details:') . "\n\n" .
                    ...
                    $this->l('Mode:') . ' ' . (Configuration::get('OPENPAY_STORE_MODE') == 'true' ? $this->l('Live') : $this->l('Test')) . "\n";
            
            $this->copyMailTemplate();
            
            $mail_detail = '<img src="'.$barcode_url.'"><br/><span style="color:#333"><strong>Referencia:</strong></span>'.$reference;
            
            $checkout = Module::getInstanceByName('openpaystores');
            ...
            if ($result_json->id){
                    ...
                    Db::getInstance()->insert('openpay_cash_transaction', array(
                        'type'          => 'payment',
                        ...
                        'mode'          => (Configuration::get('OPENPAY_STORE_MODE') == 'true' ? 'live' : 'test'),
    ...
    public function checkSettings($mode = 'global') {
        if (Configuration::get('OPENPAY_STORE_MODE')){
            return Configuration::get('OPENPAY_STORE_PUBLIC_KEY_LIVE') != '' && Configuration::get('OPENPAY_STORE_PRIVATE_KEY_LIVE') != '';
        }else{
            return Configuration::get('OPENPAY_STORE_PUBLIC_KEY_TEST') != '' && Configuration::get('OPENPAY_STORE_PRIVATE_KEY_TEST') != '';
        }
    }
    ...
    public function getContent() {
        ...
        if (Tools::isSubmit('SubmitOpenpay')) {
            ...
            if (empty($errors)) {
                $configuration_values = array(
                    'OPENPAY_STORE_MODE' => Tools::getValue('openpay_mode'),
                    'OPENPAY_STORE_MERCHANT_ID_TEST' => trim(Tools::getValue('openpay_merchant_id_test')),
                    'OPENPAY_STORE_MERCHANT_ID_LIVE' => trim(Tools::getValue('openpay_merchant_id_live')),
                    'OPENPAY_STORE_PUBLIC_KEY_TEST' => trim(Tools::getValue('openpay_public_key_test')),
                    'OPENPAY_STORE_PUBLIC_KEY_LIVE' => trim(Tools::getValue('openpay_public_key_live')),
                    'OPENPAY_STORE_PRIVATE_KEY_TEST' => trim(Tools::getValue('openpay_private_key_test')),
                    'OPENPAY_STORE_PRIVATE_KEY_LIVE' => trim(Tools::getValue('openpay_private_key_live')),
                    'OPENPAY_STORE_DEADLINE_TEST' => trim(Tools::getValue('openpay_deadline_test')),
                    'OPENPAY_STORE_DEADLINE_LIVE' => trim(Tools::getValue('openpay_deadline_live'))
                );
                ...
                $webhook_data = array(
                    'url' => (Configuration::get('PS_SSL_ENABLED') ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . __PS_BASE_URI__ . 'modules/openpaystores/notification.php',
		...
		<div class="openpay-module-wrapper">
			...
			<div class="openpay-module-header">
				...
				<span class="openpay-module-intro">' . $this->l('Comienza a recibir pagos en efectivo hoy mismo con Openpay.') . '</span>
			...
			<div class="openpay-module-wrap">
				...
				<div class="openpay-module-col2">
					...
					<div class="openpay-module-col2inner">
						<h3>' . $this->l('Múltiples puntos ubicados a lo largo de todo México') . '</h3>
						<p><img src="' . $this->_path . 'img/stores_logo.png" alt="openpay" class="openpay-cc" /> <a href="http://www.openpay.mx/tiendas-de-conveniencia.html" target="_blank" class="openpay-module-btn"><strong>' . $this->l('Tiendas afiliadas') . '</strong></a></p>
					</div>
				</div>
			</div>
			<fieldset>
		...
		<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" method="post">
			<fieldset class="openpay-settings">
				...
				<input type="radio" name="openpay_mode" value="0"' . (!Configuration::get('OPENPAY_STORE_MODE') ? ' checked="checked"' : '') . ' /> Sandbox
				<input type="radio" name="openpay_mode" value="1"' . (Configuration::get('OPENPAY_STORE_MODE') ? ' checked="checked"' : '') . ' /> Producción
				<br /><br />
					...
					<tr>
						<td align="center" valign="middle" colspan="2">
							<table cellspacing="0" cellpadding="0" class="innerTable">
									...
									<td align="left" valign="middle"><input type="text" name="openpay_merchant_id_test" value="' . Tools::safeOutput(Configuration::get('OPENPAY_STORE_MERCHANT_ID_TEST')) . '" /></td>
									<td width="15"></td>
									<td width="15" class="vertBorder"></td>
									<td align="left" valign="middle">' . $this->l('Merchant ID producción') . '</td>
									<td align="left" valign="middle"><input type="text" name="openpay_merchant_id_live" value="' . Tools::safeOutput(Configuration::get('OPENPAY_STORE_MERCHANT_ID_LIVE')) . '" /></td>
								</tr>
								<tr>
									<td align="right" valign="middle">' . $this->l('Llave pública sandbox') . '</td>
									<td align="left" valign="middle"><input type="text" name="openpay_public_key_test" value="' . Tools::safeOutput(Configuration::get('OPENPAY_STORE_PUBLIC_KEY_TEST')) . '" /></td>
									<td width="15"></td>
									<td width="15" class="vertBorder"></td>
									<td align="left" valign="middle">' . $this->l('Llave pública producción') . '</td>
									<td align="left" valign="middle"><input type="text" name="openpay_public_key_live" value="' . Tools::safeOutput(Configuration::get('OPENPAY_STORE_PUBLIC_KEY_LIVE')) . '" /></td>
								</tr>
								<tr>
									<td align="right" valign="middle">' . $this->l('Llave privada sandbox') . '</td>
									<td align="left" valign="middle"><input type="password" name="openpay_private_key_test" value="' . Tools::safeOutput(Configuration::get('OPENPAY_STORE_PRIVATE_KEY_TEST')) . '" /></td>
									<td width="15"></td>
									<td width="15" class="vertBorder"></td>
									<td align="left" valign="middle">' . $this->l('Llave privada producción') . '</td>
									<td align="left" valign="middle"><input type="password" name="openpay_private_key_live" value="' . Tools::safeOutput(Configuration::get('OPENPAY_STORE_PRIVATE_KEY_LIVE')) . '" /></td>
								</tr>
                                                                <tr>
									<td align="right" valign="middle">' . $this->l('Fecha límite de pago (horas)') . '</td>
									<td align="left" valign="middle"><input type="text" name="openpay_deadline_test" value="' . Tools::safeOutput(Configuration::get('OPENPAY_STORE_DEADLINE_TEST')) . '" /></td>
									<td width="15"></td>
									<td width="15" class="vertBorder"></td>
									<td align="left" valign="middle">' . $this->l('Fecha límite de pago (horas)') . '</td>
									<td align="left" valign="middle"><input type="text" name="openpay_deadline_live" value="' . Tools::safeOutput(Configuration::get('OPENPAY_STORE_DEADLINE_LIVE')) . '" /></td>
								</tr>
							</table>
						</td>
					</tr>
    ...
    public function getCustomer($customer_id){
        ...
        $pk = Configuration::get('OPENPAY_STORE_MODE') ? Configuration::get('OPENPAY_STORE_PRIVATE_KEY_LIVE') : Configuration::get('OPENPAY_STORE_PRIVATE_KEY_TEST');
        ...
        Openpay::setProductionMode(Configuration::get('OPENPAY_STORE_MODE'));   
    ...
    public function copyMailTemplate(){
        ...
        $html_file_origin = _PS_MODULE_DIR_ .$this->name. '/mails/es/openpaystores.html';
        $html_file_destination = dirname(__FILE__).'/../../mails/'.$lang.'/openpaystores.html';
        if(!copy($html_file_origin, $html_file_destination)){
            throw new Exception;
        }

        $txt_file_origin = _PS_MODULE_DIR_ .$this->name .'/mails/es/openpaystores.txt';
        $txt_file_destination = dirname(__FILE__).'/../../mails/'.$lang.'/openpaystores.txt';
